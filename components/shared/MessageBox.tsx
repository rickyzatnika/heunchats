  ...
  return message?.sender?._id !== currentUser?._id ? (
    <div className="flex gap-3 items-start">
      ...
      <div className="flex flex-col gap-2">
        ...
        ${/^[\p{Emoji}\s]+$/u.test(message.text) ? "bg-transparent text-xl" : "bg-muted dark:bg-muted/40 text-muted-foreground"}`}
          >
            {message.text}
          </p>
        ) : (
    ...
    <div className="flex gap-3 items-start justify-end">
      <div className="flex flex-col gap-2 items-end">
        ...
        {message?.text ? (
          <p
            className={`w-fit text-sm md:text-md px-3 py-1.5 rounded-full 
        ${/^[\p{Emoji}\s]+$/u.test(message.text) ? "bg-transparent text-xl" : "bg-primary/60 text-primary-foreground"}`}
          >
            {message.text}
          </p>
        ) : (
